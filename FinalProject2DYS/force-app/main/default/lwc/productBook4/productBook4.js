...
import BookImage4 from'@salesforce/resourceUrl/BookImage4';
...
export default class ProductBook4 extends LightningElement {
    ...
    @wire(getPro)proList({data,error}){
        if(data){
            ...
            this.pname=data[7].Name;
            this.pprice=data[7].price__c;
            this.pdescription=data[7].Description__c;
            this.proId=data[7].prod_id__c;
            this.pstock=data[7].StockAvailable__c;
        }
        else if(error){
            console.log(error);
        }

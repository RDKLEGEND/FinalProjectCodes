...
import WatchElectronic from'@salesforce/resourceUrl/WatchElectronic';
...
export default class ProductWatch extends LightningElement {
    ...
    @wire(getPro)proList({data,error}){
        if(data){
            ...
            this.pname=data[1].Name;
            this.pprice=data[1].price__c;
            this.pdescription=data[1].Description__c;
            this.proId=data[1].prod_id__c;
            this.pstock=data[1].StockAvailable__c;
        }
        else if(error){
            console.log(error);
        }
